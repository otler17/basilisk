...
#. In the Project tab, navigate to ``Plugins/crosstales/FileBrowser/Resources/Prefabs/``
   and move the ``FileBrowser`` prefab into ``Resources/Prefabs/``.
#. Select the ``FileBrowser`` prefab in the Project panel so it becomes active in the Inspector.
#. In the Inspector, enable ``Allow Sync Calls`` under the macOS settings.
